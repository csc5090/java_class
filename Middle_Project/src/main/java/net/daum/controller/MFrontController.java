...
public class MFrontController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) 
		...
		try {
			// [오류 수정 1: new FileInputStream()의 결과를 fis 변수에 할당]
			fis = new FileInputStream(request.getSession().getServletContext().getRealPath(""
		...
		prop.load(fis);//프로퍼티 파일 로드 실행
			...
			e.printStackTrace();
			return; // 파일 로드 실패 시 응답 없이 종료되어 404 원인이 됩니다.
		} finally {
			if (fis != null) {
				fis.close(); // [오류 수정 2: 리소스 반납은 finally에서]
			}
		}
		
		value = prop.getProperty(command);//키이름인 매핑주소에 대한 값을 구함
		
		// [오류 수정 3: value가 null인지 확인하고, substring() 호출 전에 길이 체크]
		if(value != null && value.length() >= 7 && value.substring(0,7).equals("execute")) {//0이상 7미만 사이의 문자가 execute와 같다면
			...
			try {
				...
				// [오류 수정 4: Deprecated된 newInstance() 대신 권장되는 생성자 호출 방식을 사용]
				action=(Action)url.getDeclaredConstructor().newInstance();//새로운 인스턴스로 다운캐스팅 해서 액션 생성
				...
				ie.printStackTrace(); // [수정: InstantiationException은 ClassNotFoundException과 같은 ce 변수를 쓰지 않도록 함]
			}catch(IllegalAccessException ae) {//클래스에 접근 못할 때 -> 클래스의 필드(멤버변수,속성),
				//메서드,생성자 등에 접근 권한이 없을 경우 발생
				ae.printStackTrace();
			}catch(Exception e) { // [추가: 모든 Exception을 잡아 디버깅 정보 확보]
                e.printStackTrace();
            }
		}//if
